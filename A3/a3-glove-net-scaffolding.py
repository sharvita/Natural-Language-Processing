...
optimizer = torch.optim.Adam(gloveModel.parameters(), lr=learning_rate)
...
with torch.no_grad():
    test_outputs = gloveModel(test_inputs)
...
print(k_nearest_neighbors(gloveModel.embedding.weight.data,token2index,'actor'))

plot_embeddings_3d(gloveModel.embedding.weight.data,token2index,index2token)

plot_embeddings_tsne(gloveModel.embedding.weight.data, token2index, index2token)